...
FULL_DECK = {"Ace of Hearts" => [1, 11], "Ace of Spades" => [1, 11], "Ace of Clubs" => [1, 11], "Ace of Diamonds" => [1, 11], 
...
def card_total(holders_cards)
	...
	if holders_cards.any? { |card| card.include?("Ace") }
		holders_cards.each do |card| 
			ace_total.each_with_index { |value, index| value + FULL_DECK[card][index] }
		end	
		return ace_total
	else
		holders_cards.each { |card| total = total + FULL_DECK[card] }
		return total
	end
end
